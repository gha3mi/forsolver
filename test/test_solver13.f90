...
end module my_function13

program test_solver13

   use forsolver_kinds, only: rk
   use forsolver, only: nlsolver
   use my_function13, only: F4
   ...
   call ut%check(real(x,rk), expected_x, 1.0e-1_rk, 'test_solver13' )
   ! call ut%check(x%re, expected_x, 1.0e-1_rk, 'test_solver13' )

end program test_solver13
